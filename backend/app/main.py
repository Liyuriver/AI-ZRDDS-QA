...
from app.api.user import auth_router, router as user_router
...
AVATAR_DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "avatars"
HYBRID_DATA_DIR.mkdir(parents=True, exist_ok=True)
AVATAR_DATA_DIR.mkdir(parents=True, exist_ok=True)
app.mount(
    "/static/hybrid",
...
app.mount(
    "/static/avatars",
    StaticFiles(directory=str(AVATAR_DATA_DIR)),
    name="avatar-static",
)
